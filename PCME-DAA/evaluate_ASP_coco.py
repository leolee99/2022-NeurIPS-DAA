...
@torch.no_grad()
def evaluate(config, model_path, dataloader, logger, n_crossfolds):
...
def main(config_path,
    ...
    torch.cuda.set_device(config.test.gpus)
    logger = PythonLogger()
    ...
    scores = evaluate(config, model_path, dataloader, logger, n_crossfolds)
    # if dump_to:
