...
def image_to_caption_collate_fn(data):
    ...
    images, sentences, ann_ids, image_ids, hash_ids, tfidf_single = zip(*data)
    ...
    targets = torch.zeros(len(sentences), 80).long()
    for i, cap in enumerate(sentences):
        end = cap_lengths[i]
        targets[i, :end] = cap[:end]

    hash_ids = torch.Tensor(hash_ids)
    tfidf_single = torch.Tensor(tfidf_single)
    cap_lengths = torch.Tensor(cap_lengths).long()
    return images, targets, cap_lengths, ann_ids, image_ids, hash_ids, tfidf_single 
