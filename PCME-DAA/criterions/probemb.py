...
from criterions.DAA import DAA
...
class MCSoftContrastiveLoss(nn.Module):
    ...
    def __init__(self, config, reduction='sum'):
        ...
        self.daa_beta = config.get('daa_beta', 0)
        self.ap_beta = config.get('ap_beta', 0)

        N = config.batch_size * config.num_samples
        self.matched = torch.eye(N)
        for i in range(0, N, self.num_samples):
            self.matched[i:i + self.num_samples, i:i + self.num_samples] = 1
    ...
    def _compute_loss(self, input1, input2, cider_map=None, mode='i2t'):
        """
        Shape
        -----
        Input1 : torch.Tensor
        ...
        cider_loss = torch.tensor(0)
        distance, matched = self.pairwise_sampling(input1, input2)

        if self.ap_beta:
            ap_loss = self.sem.Smooth_AP(input1, input2)
        if self.daa_beta:
            cider_loss = self.sem.coco_DAA(input1, input2, cider_map)
        logits = -self.negative_scale * distance + self.shift

        idx = matched == 1
        loss_pos = soft_contrastive_nll(logits[idx], matched[idx])
        loss_pos = loss_pos.sum()
        idx = matched != 1
        loss_neg = soft_contrastive_nll(logits[idx], matched[idx])
    ...
    def forward(self, image_features, caption_features, image_logsigma, caption_logsigma, sem_map=None, **kwargs):
        uniform_loss = 0
        uniform_loss_val = 0
        vib_loss = 0
        vib_loss_val = 0
        ...
        cider_map = self.sem.coco_cider_compute(sem_map)

        i2t_loss = self._compute_loss(sampled_image_features, sampled_caption_features, cider_map, mode='i2t')
        t2i_loss = self._compute_loss(sampled_caption_features, sampled_image_features, cider_map.t(), mode='t2i')
        loss = i2t_loss['loss'] + t2i_loss['loss'] + self.uniform_lambda * uniform_loss + self.vib_beta * vib_loss + self.ap_beta * i2t_loss['ap_loss'] + self.ap_beta * t2i_loss['ap_loss'] + self.daa_beta * i2t_loss['cider_loss'] + self.daa_beta * t2i_loss['cider_loss']
        ...
        loss_dict = {'i2t_loss': i2t_loss['loss'].item(),
                     ...
                     'i2t_ap_loss': (self.ap_beta * i2t_loss['ap_loss']).item(),
                     't2i_ap_loss': (self.ap_beta * t2i_loss['ap_loss']).item(),
                     'i2t_cider_loss': (self.daa_beta * i2t_loss['cider_loss']).item(),
                     't2i_cider_loss': (self.daa_beta * t2i_loss['cider_loss']).item(),
