...
def prepare_data(dataset_root, ids, n_fold_idx, ds=None):
    ...
    if ds is None:
        ...
        ds = CocoCaptionsCap(root, annotation_path, ids=ids, instance_annFile=instance_annotation_path, tokenizer=tokenizer)
    if n_fold_idx > -1:
        image_ids = []
        seen_iid = set()
        for idx in tqdm(range(len(ds)), total=len(ds)):
            _, _, _, image_id, _, _ = ds[idx]
...
def main(ranking_fname, dataset_root, n_fold=None, max_thres=3):
    ...
    print('\n------------------------------')
    # print(f'image-to-text PMR1: {np.mean(i2t_pmr1s)}')
    # print(f'text-to-image PMR1: {np.mean(t2i_pmr1s)}')
    # print(f'image-to-text PMR1 thres:2: {np.mean(list(i2t_thres_pmr1s))}')
    # print(f'text-to-image PMR1 thres:2:: {np.mean(list(t2i_thres_pmr1s))}')
