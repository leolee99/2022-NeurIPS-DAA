...
class EngineBase(object):
    ...
    def create(self, config, evaluator, verbose=False):
        self.config = config
    ...
    @torch.no_grad()
    def ASP_evaluate(self, val_loaders, n_crossfolds=None, **kwargs):
        if self.evaluator is None:
            self.logger.log('[Evaluate] Warning, no evaluator is defined. Skip evaluation')
            return
        ...
        for key, data_loader in val_loaders.items():
            self.logger.log('Evaluating {}...'.format(key))
            _n_crossfolds = -1 if key == 'val' else n_crossfolds
            scores[key] = self.evaluator.evaluate_ASP(data_loader, n_crossfolds=_n_crossfolds,
                                                  key=key, **kwargs)
        return scores
