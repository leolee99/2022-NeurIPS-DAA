...
class EncoderImage(nn.Module):
    ...
    def forward(self, images):
        out_7x7 = self.cnn(images).view(-1, self.cnn_dim, 7, 7)  # batchsize 1024 7 7
        pooled = self.avgpool(out_7x7).view(-1, self.cnn_dim)  # batchsize 1024 #mean
        out = self.fc(pooled)  # batchsize 1024

        output = {}
        out_7x7 = out_7x7.view(-1, self.cnn_dim, 7 * 7)  # batchsize 1024 7*7

        if self.use_attention:
            out, attn, residual = self.pie_net(out, out_7x7.transpose(1, 2))  # compute mean
            output['attention'] = attn
            output['residual'] = residual

        if self.use_probemb:
            uncertain_out = self.uncertain_net(pooled, out_7x7.transpose(1, 2))   # compute variance
