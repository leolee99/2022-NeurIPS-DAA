...
from semantic import cider_compute, bleu_compute, meteor_compute, rouge_compute, cider_compute_one
...
class Loss(nn.Module):
    ...
    def sigmoid(self, tensor, temp=0.01):
    ...
    def DAA(self, scores, sems):
        N = len(scores)

        score_broad = scores.unsqueeze(-1) #[128, 1, 128]
        score_expand = scores[None, :, :]
        score_diff = score_broad - score_expand

        sem_broad = sems.unsqueeze(-1)
        sem_expand = sems[None, :, :]
        sem_diff = sem_broad - sem_expand

        diag_mask = (1 - torch.eye(N)).unsqueeze(1).to('cuda')  #mask diag to 0

        sem_rank = diag_mask * self.sigmoid(tensor = sem_diff, temp = 0.001)
        sem_rank_all = sem_rank.sum(0) + 1
        score_rank = diag_mask * self.sigmoid(tensor = score_diff, temp = 0.001)
        score_rank_all = score_rank.sum(0) + 1
    
        min_rank = torch.min(sem_rank_all, score_rank_all)
        max_rank = torch.max(sem_rank_all, score_rank_all)
    
        ASP = (min_rank / max_rank).sum() / (N**2)

        return 50 * (1 - ASP)

    def SmoothAP(self, scores):
        N = len(scores)  #batchsize

        score_broad = scores.t().unsqueeze(-1)
        score_expand = scores[None, :, :]
        score_diff = score_broad - score_expand

        matched = torch.eye(N).to('cuda')
        diag_mask = (1 - torch.eye(N)).unsqueeze(1).to('cuda')  #mask diag to 0
        ...
        AP = (score_rank_pos / score_rank_all).sum() / N

        return 25 * (1 - AP)

    def forward(self, scores, sems=None):
        SA = False
        contrastive_loss, DAA_i2t_loss, DAA_t2i_loss = torch.zeros([1],dtype=torch.float).cuda(), torch.zeros([1],dtype=torch.float).cuda(), torch.zeros([1],dtype=torch.float).cuda()
        SA_i2t_loss, SA_t2i_loss = torch.zeros([1],dtype=torch.float).cuda(), torch.zeros([1],dtype=torch.float).cuda()
        contrastive_loss = self.contrastive(scores)

        if sems != None:
            DAA_i2t_loss = self.DAA(scores, sems)
            DAA_t2i_loss = self.DAA(scores.t(), sems.t())
        
        if SA:
            SA_i2t_loss = self.SmoothAP(scores)
            SA_t2i_loss = self.SmoothAP(scores.t())

        loss = contrastive_loss + DAA_i2t_loss + DAA_t2i_loss + SA_i2t_loss + SA_t2i_loss 
        return {
            'contrastive_loss': contrastive_loss,
            'DAA_i2t_loss': DAA_i2t_loss,
            'DAA_t2i_loss': DAA_t2i_loss,
            'SA_i2t_loss': SA_i2t_loss,
            'SA_t2i_loss': SA_t2i_loss,
            'loss': loss,
        }
...
class SGRAF(object):
    ...
    def __init__(self, opt, rank=0):
        ...
        self.img_enc = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.img_enc)
        self.txt_enc = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.txt_enc)
        self.sim_enc = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.sim_enc)
        self.img_enc = torch.nn.parallel.DistributedDataParallel(self.img_enc, device_ids=[rank], output_device=rank)
        self.txt_enc = torch.nn.parallel.DistributedDataParallel(self.txt_enc, device_ids=[rank], output_device=rank)
        self.sim_enc = torch.nn.parallel.DistributedDataParallel(self.sim_enc, device_ids=[rank], output_device=rank)
    ...
    def forward_loss(self, sims, sems=None, **kwargs):
        """Compute the loss given pairs of image and caption embeddings
        """
        loss = self.criterion(sims, sems)
        for key in loss.keys():
            self.logger.update(key, loss[key].item(), sims.size(0))
        return loss['loss']

    def train_emb(self, images, captions, lengths, ids=None, tfidf_GT=None, tfidf_single=None, *args):
        ...
        sems = None
        #sems = cider_compute_one((tfidf_GT, tfidf_single))
        #sems = cider_compute((tfidf_GT, tfidf_single))
        #sems = bleu_compute((tfidf_GT, tfidf_single))
        #sems = meteor_compute((tfidf_GT, tfidf_single))
        #sems = rouge_compute((tfidf_GT, tfidf_single))

        # measure accuracy and record loss
        self.optimizer.zero_grad()
        loss = self.forward_loss(sims, sems)
